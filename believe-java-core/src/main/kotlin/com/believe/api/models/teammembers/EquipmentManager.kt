...
class EquipmentManager @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (characterId.asKnown().isPresent) 1 else 0) + (if (teamId.asKnown().isPresent) 1 else 0) + (if (yearsWithTeam.asKnown().isPresent) 1 else 0) + (if (isHeadKitman.asKnown().isPresent) 1 else 0) + (memberType.asKnown().getOrNull()?.validity() ?: 0) + (responsibilities.asKnown().getOrNull()?.size ?: 0)
