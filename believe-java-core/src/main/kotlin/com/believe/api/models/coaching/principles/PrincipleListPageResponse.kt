...
class PrincipleListPageResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (data.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (hasMore.asKnown().isPresent) 1 else 0) + (if (limit.asKnown().isPresent) 1 else 0) + (if (page.asKnown().isPresent) 1 else 0) + (if (pages.asKnown().isPresent) 1 else 0) + (if (skip.asKnown().isPresent) 1 else 0) + (if (total.asKnown().isPresent) 1 else 0)
