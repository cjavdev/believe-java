...
class Biscuit @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (message.asKnown().isPresent) 1 else 0) + (if (pairsWellWith.asKnown().isPresent) 1 else 0) + (if (tedNote.asKnown().isPresent) 1 else 0) + (type.asKnown().getOrNull()?.validity() ?: 0) + (if (warmthLevel.asKnown().isPresent) 1 else 0)
