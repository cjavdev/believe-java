...
class BelieveSubmitParams private constructor(
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (situation.asKnown().isPresent) 1 else 0) + (situationType.asKnown().getOrNull()?.validity() ?: 0) + (if (context.asKnown().isPresent) 1 else 0) + (if (intensity.asKnown().isPresent) 1 else 0)
