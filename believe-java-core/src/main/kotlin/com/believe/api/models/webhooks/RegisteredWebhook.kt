...
class RegisteredWebhook @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (createdAt.asKnown().isPresent) 1 else 0) + (eventTypes.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (secret.asKnown().isPresent) 1 else 0) + (if (url.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0)
