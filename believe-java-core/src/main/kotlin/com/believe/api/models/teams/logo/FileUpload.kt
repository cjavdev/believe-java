...
class FileUpload @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (checksumSha256.asKnown().isPresent) 1 else 0) + (if (contentType.asKnown().isPresent) 1 else 0) + (if (fileId.asKnown().isPresent) 1 else 0) + (if (filename.asKnown().isPresent) 1 else 0) + (if (sizeBytes.asKnown().isPresent) 1 else 0) + (if (uploadedAt.asKnown().isPresent) 1 else 0)
