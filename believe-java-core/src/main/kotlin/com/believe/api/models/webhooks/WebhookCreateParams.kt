...
class WebhookCreateParams private constructor(
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (url.asKnown().isPresent) 1 else 0) + (if (description.asKnown().isPresent) 1 else 0) + (eventTypes.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0)
