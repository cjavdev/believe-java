...
class ConflictResolveResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (barbecueSauceWisdom.asKnown().isPresent) 1 else 0) + (if (diagnosis.asKnown().isPresent) 1 else 0) + (if (diamondDogsAdvice.asKnown().isPresent) 1 else 0) + (if (potentialOutcome.asKnown().isPresent) 1 else 0) + (stepsToResolution.asKnown().getOrNull()?.size ?: 0) + (if (tedApproach.asKnown().isPresent) 1 else 0)
