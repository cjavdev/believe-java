...
class PressSimulateResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
    ...
    class Builder internal constructor() {
        ...
        fun deflectionHumor(deflectionHumor: Optional<String>) = deflectionHumor(deflectionHumor.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (actualWisdom.asKnown().isPresent) 1 else 0) + (if (followUpDodge.asKnown().isPresent) 1 else 0) + (if (reporterReaction.asKnown().isPresent) 1 else 0) + (if (response.asKnown().isPresent) 1 else 0) + (if (deflectionHumor.asKnown().isPresent) 1 else 0)
