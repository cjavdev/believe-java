...
class Character @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws BelieveInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun salaryGbp(): Optional<String> = salaryGbp.getOptional("salary_gbp")
    ...
    class Builder internal constructor() {
        ...
        fun salaryGbp(salaryGbp: Optional<String>) = salaryGbp(salaryGbp.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (background.asKnown().isPresent) 1 else 0) + (emotionalStats.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (personalityTraits.asKnown().getOrNull()?.size ?: 0) + (role.asKnown().getOrNull()?.validity() ?: 0) + (if (dateOfBirth.asKnown().isPresent) 1 else 0) + (if (email.asKnown().isPresent) 1 else 0) + (growthArcs.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (heightMeters.asKnown().isPresent) 1 else 0) + (if (profileImageUrl.asKnown().isPresent) 1 else 0) + (if (salaryGbp.asKnown().isPresent) 1 else 0) + (signatureQuotes.asKnown().getOrNull()?.size ?: 0) + (if (teamId.asKnown().isPresent) 1 else 0)
