...
class MedicalStaff @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (characterId.asKnown().isPresent) 1 else 0) + (specialty.asKnown().getOrNull()?.validity() ?: 0) + (if (teamId.asKnown().isPresent) 1 else 0) + (if (yearsWithTeam.asKnown().isPresent) 1 else 0) + (if (licenseNumber.asKnown().isPresent) 1 else 0) + (memberType.asKnown().getOrNull()?.validity() ?: 0) + (qualifications.asKnown().getOrNull()?.size ?: 0)
