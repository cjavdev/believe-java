...
class WebhookTriggerEventResponse @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (deliveries.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (eventId.asKnown().isPresent) 1 else 0) + (eventType.asKnown().getOrNull()?.validity() ?: 0) + (if (successfulDeliveries.asKnown().isPresent) 1 else 0) + (if (tedSays.asKnown().isPresent) 1 else 0) + (if (totalWebhooks.asKnown().isPresent) 1 else 0)
    ...
    class Delivery @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (success.asKnown().isPresent) 1 else 0) + (if (url.asKnown().isPresent) 1 else 0) + (if (webhookId.asKnown().isPresent) 1 else 0) + (if (error.asKnown().isPresent) 1 else 0) + (if (statusCode.asKnown().isPresent) 1 else 0)
