...
class TicketSaleUpdateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        fun purchaseMethod(purchaseMethod: Optional<PurchaseMethod>) = purchaseMethod(purchaseMethod.getOrNull())
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         * @throws BelieveInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
         */
        fun purchaseMethod(): Optional<PurchaseMethod> = purchaseMethod.getOptional("purchase_method")
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (buyerEmail.asKnown().isPresent) 1 else 0) + (if (buyerName.asKnown().isPresent) 1 else 0) + (if (couponCode.asKnown().isPresent) 1 else 0) + (if (currency.asKnown().isPresent) 1 else 0) + (if (discount.asKnown().isPresent) 1 else 0) + (if (matchId.asKnown().isPresent) 1 else 0) + (purchaseMethod.asKnown().getOrNull()?.validity() ?: 0) + (if (quantity.asKnown().isPresent) 1 else 0) + (if (subtotal.asKnown().isPresent) 1 else 0) + (if (tax.asKnown().isPresent) 1 else 0) + (if (total.asKnown().isPresent) 1 else 0) + (if (unitPrice.asKnown().isPresent) 1 else 0)
