...
class Quote @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (characterId.asKnown().isPresent) 1 else 0) + (if (context.asKnown().isPresent) 1 else 0) + (momentType.asKnown().getOrNull()?.validity() ?: 0) + (if (text.asKnown().isPresent) 1 else 0) + (theme.asKnown().getOrNull()?.validity() ?: 0) + (if (episodeId.asKnown().isPresent) 1 else 0) + (if (isFunny.asKnown().isPresent) 1 else 0) + (if (isInspirational.asKnown().isPresent) 1 else 0) + (if (popularityScore.asKnown().isPresent) 1 else 0) + (secondaryThemes.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (timesShared.asKnown().isPresent) 1 else 0)
