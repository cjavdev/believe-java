...
class CoachingPrinciple @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (application.asKnown().isPresent) 1 else 0) + (if (exampleFromShow.asKnown().isPresent) 1 else 0) + (if (explanation.asKnown().isPresent) 1 else 0) + (if (principle.asKnown().isPresent) 1 else 0) + (if (tedQuote.asKnown().isPresent) 1 else 0)
