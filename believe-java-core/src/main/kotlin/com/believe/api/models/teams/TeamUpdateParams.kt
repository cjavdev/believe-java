...
class TeamUpdateParams private constructor(
    ...
    class Builder internal constructor() {
        ...
        fun annualBudgetGbp(annualBudgetGbp: Optional<AnnualBudgetGbp>) = annualBudgetGbp(annualBudgetGbp.getOrNull())
        ...
        fun annualBudgetGbp(number: Double) =
            apply {
                body.annualBudgetGbp(number)
            }
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
        ...
        /** @throws BelieveInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value). */
        fun annualBudgetGbp(): Optional<AnnualBudgetGbp> = annualBudgetGbp.getOptional("annual_budget_gbp")
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (annualBudgetGbp.asKnown().getOrNull()?.validity() ?: 0) + (if (averageAttendance.asKnown().isPresent) 1 else 0) + (if (contactEmail.asKnown().isPresent) 1 else 0) + (if (cultureScore.asKnown().isPresent) 1 else 0) + (if (foundedYear.asKnown().isPresent) 1 else 0) + (if (isActive.asKnown().isPresent) 1 else 0) + (league.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (if (nickname.asKnown().isPresent) 1 else 0) + (if (primaryColor.asKnown().isPresent) 1 else 0) + (rivalTeams.asKnown().getOrNull()?.size ?: 0) + (if (secondaryColor.asKnown().isPresent) 1 else 0) + (if (stadium.asKnown().isPresent) 1 else 0) + (stadiumLocation.asKnown().getOrNull()?.validity() ?: 0) + (values.asKnown().getOrNull()?.validity() ?: 0) + (if (website.asKnown().isPresent) 1 else 0) + (if (winPercentage.asKnown().isPresent) 1 else 0)
    ...
    class AnnualBudgetGbp private constructor(
        ...
        companion object {
            ...
            @JvmStatic
            fun ofNumber(number: Double) = AnnualBudgetGbp(number = number)
