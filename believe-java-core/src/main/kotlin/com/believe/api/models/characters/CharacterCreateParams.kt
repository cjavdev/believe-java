...
class CharacterCreateParams private constructor(
    ...
    class Body @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
         ...
         */
        @JvmSynthetic
        internal fun validity(): Int = (if (background.asKnown().isPresent) 1 else 0) + (emotionalStats.asKnown().getOrNull()?.validity() ?: 0) + (if (name.asKnown().isPresent) 1 else 0) + (personalityTraits.asKnown().getOrNull()?.size ?: 0) + (role.asKnown().getOrNull()?.validity() ?: 0) + (if (dateOfBirth.asKnown().isPresent) 1 else 0) + (if (email.asKnown().isPresent) 1 else 0) + (growthArcs.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (heightMeters.asKnown().isPresent) 1 else 0) + (if (profileImageUrl.asKnown().isPresent) 1 else 0) + (salaryGbp.asKnown().getOrNull()?.validity() ?: 0) + (signatureQuotes.asKnown().getOrNull()?.size ?: 0) + (if (teamId.asKnown().isPresent) 1 else 0)
    ...
    class SalaryGbp private constructor(
        ...
        override fun toString(): String =
            when {
                number != null -> "SalaryGbp{number=$number}"
