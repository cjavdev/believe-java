...
class Player @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (characterId.asKnown().isPresent) 1 else 0) + (if (jerseyNumber.asKnown().isPresent) 1 else 0) + (position.asKnown().getOrNull()?.validity() ?: 0) + (if (teamId.asKnown().isPresent) 1 else 0) + (if (yearsWithTeam.asKnown().isPresent) 1 else 0) + (if (assists.asKnown().isPresent) 1 else 0) + (if (goalsScored.asKnown().isPresent) 1 else 0) + (if (isCaptain.asKnown().isPresent) 1 else 0) + (memberType.asKnown().getOrNull()?.validity() ?: 0)
