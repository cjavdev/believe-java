...
class Match @JsonCreator(mode = JsonCreator.Mode.DISABLED) private constructor(
     ...
     * @throws BelieveInvalidDataException if the JSON field has an unexpected type (e.g. if the server responded with an unexpected value).
     */
    fun ticketRevenueGbp(): Optional<String> = ticketRevenueGbp.getOptional("ticket_revenue_gbp")
    ...
    class Builder internal constructor() {
        ...
        /** Alias for calling [Builder.attendance] with `attendance.orElse(null)`. */
        fun attendance(attendance: Optional<Long>) = attendance(attendance.getOrNull())
        ...
        fun ticketRevenueGbp(ticketRevenueGbp: Optional<String>) = ticketRevenueGbp(ticketRevenueGbp.getOrNull())
     ...
     */
    @JvmSynthetic
    internal fun validity(): Int = (if (id.asKnown().isPresent) 1 else 0) + (if (awayTeamId.asKnown().isPresent) 1 else 0) + (if (date.asKnown().isPresent) 1 else 0) + (if (homeTeamId.asKnown().isPresent) 1 else 0) + (matchType.asKnown().getOrNull()?.validity() ?: 0) + (if (attendance.asKnown().isPresent) 1 else 0) + (if (awayScore.asKnown().isPresent) 1 else 0) + (if (episodeId.asKnown().isPresent) 1 else 0) + (if (homeScore.asKnown().isPresent) 1 else 0) + (if (lessonLearned.asKnown().isPresent) 1 else 0) + (if (possessionPercentage.asKnown().isPresent) 1 else 0) + (result.asKnown().getOrNull()?.validity() ?: 0) + (if (tedHalftimeSpeech.asKnown().isPresent) 1 else 0) + (if (ticketRevenueGbp.asKnown().isPresent) 1 else 0) + (turningPoints.asKnown().getOrNull()?.sumOf { it.validity().toInt() } ?: 0) + (if (weatherTempCelsius.asKnown().isPresent) 1 else 0)
